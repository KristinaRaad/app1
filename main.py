...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main lifestyle: { “on the move”, “office”, “workout”, “healthy”, “smoothy lover”, “water drinker” };
    TASK: Write a product description that is tailored into this customer's Age group and lifestyle. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and lifestyle;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account lifestyle {lifestyle} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "lifestyle", "content"],
    template=template,
)
...
with col1:
    st.markdown("Otstarve: Veebilehel kuvatavate tootetutvustustekstide personaliseerimine igale kliendile või kliendigruppidele; väljundtekst on kohandatud kliendi a) vanuserühmaga ja b) kliendi elustiili valikuga; sisendtekstiks on neutraalses vormis tootekirjeldus.\
    \n\n Kasutusjuhend: 1) valmista ette tootekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja elustiili kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
    4) kopeeri ükshaaval tarbijasegmentide lõikes äpi väljundteksti kõnealuse toote tutvustuslehele.")

with col2:
    st.image(image='companylogo.jpg', caption='Reusable cups for everybody')
...
def get_lifestyle():
    input_text = st.text_input(label="Customers lifestyle", key="lifestyle_input")
    return input_text

lifestyle_input = get_lifestyle()
...
def update_text_with_example():
    print ("in updated")
    st.session_state.content_input = "inlusated cups, water bottles, straw cups, glass cups"
